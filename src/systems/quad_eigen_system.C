...
// Local includes
#include "libmesh/quad_eigen_system.h"
...
QuadEigenSystem::QuadEigenSystem (EquationSystems & es,
                          const std::string & name_in,
                          const unsigned int number_in
                          ) :
  Parent           (es, name_in, number_in),
  eigen_solver     (QuadEigenSolver<Number>::build(es.comm())),
  _n_converged_eigenpairs (0),
  _n_iterations           (0),
  _eigen_problem_type (PEPGEN)
{
}


QuadEigenSystem::~QuadEigenSystem ()
{
  // clear data
  this->clear();
}
...
void QuadEigenSystem::set_eigenproblem_type (EigenProblemType ept)
{
...
void QuadEigenSystem::init_data ()
{
...
void QuadEigenSystem::init_matrices ()
{
  ...
  matrix_C.reset(SparseMatrix<Number>::build(this->comm()).release());
  dof_map.attach_matrix(*matrix_C);
...
void QuadEigenSystem::solve ()
{
  ...
  // Generalized eigenproblem
    solve_data = eigen_solver->solve_quadratic (*matrix_A, *matrix_B, *matrix_C, nev, ncv, tol, maxits);
...
std::pair<Real, Real> QuadEigenSystem::get_eigenpair (dof_id_type i)
{
