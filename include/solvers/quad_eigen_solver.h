 ...
 */
template <typename T>
class QuadEigenSolver : public ReferenceCountedObject<QuadEigenSolver<T> >,
...
public:
   ...
   */
  QuadEigenSolver (const Parallel::Communicator & comm_in
               LIBMESH_CAN_DEFAULT_TO_COMMWORLD);

  /**
   * Destructor.
   */
  virtual ~QuadEigenSolver ();

  /**
   * Builds an \p EigenSolver using the linear solver package specified by
   * \p solver_package
   */
  static UniquePtr<QuadEigenSolver<T> > build(const Parallel::Communicator & comm_in
                                          LIBMESH_CAN_DEFAULT_TO_COMMWORLD,
   ...
   * of iterations.
   */
  virtual std::pair<unsigned int, unsigned int> solve_quadratic (SparseMatrix<T> & matrix_A,
...
template <typename T>
inline
QuadEigenSolver<T>::QuadEigenSolver (const Parallel::Communicator & comm_in) :
  ParallelObject(comm_in),
  _eigen_solver_type    (TOAR),
...
template <typename T>
inline
QuadEigenSolver<T>::~QuadEigenSolver ()
{
  this->clear ();
}
